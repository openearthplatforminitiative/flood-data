import unittest
from flood.etl.utils import restrict_dataset_area
from test.data.data_generation import generate_restrict_dataset_area_test_dataset
...
class TestRestrictArea(unittest.TestCase):
    ...
    def test_restrict_dataset_area(self):
        ...
        ds = generate_restrict_dataset_area_test_dataset(lat_min, lat_max, 
                                                         lon_min, lon_max,
                                                         data_resolution)
        ...
        restricted_ds = restrict_dataset_area(ds, r_lat_min, r_lat_max, r_lon_min, r_lon_max, buffer_value)
        
        # Assert that the restricted dataset boundaries are within the expected ranges
        self.assertGreater(restricted_ds.latitude.min().item(), r_lat_min - buffer_value)
        self.assertLess(restricted_ds.latitude.max().item(), r_lat_max + buffer_value)
        self.assertGreater(restricted_ds.longitude.min().item(), r_lon_min - buffer_value)
        ...
        self.assertAlmostEqual(restricted_ds.latitude.min().item(), r_lat_min + data_resolution / 2)
        self.assertAlmostEqual(restricted_ds.latitude.max().item(), r_lat_max - data_resolution / 2)
        self.assertAlmostEqual(restricted_ds.longitude.min().item(), r_lon_min + data_resolution / 2)
        self.assertAlmostEqual(restricted_ds.longitude.max().item(), r_lon_max - data_resolution / 2)